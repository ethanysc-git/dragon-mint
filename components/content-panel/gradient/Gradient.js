...
const Gradient = ({ firstName, lastName }) => {
  ...
  const generateColor = (value, alpha) => {
    ...
    const hue = (value % 60) * 6;
    // Hues from 0 to 360 in steps of 6 (avoiding dark/brown shades)
    const saturation = 50 + (value % 31);
    // 50-80 for pastel colors
    const lightness = 70 + (value % 11);
    // 70-80 for brighter colors
